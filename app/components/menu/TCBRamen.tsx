...
function TCBRamen({ showTitle }: { showTitle?: boolean }) {
  ...
  return (
    <section className="px-4 container mx-auto py-12">
      <section className="">
        {showTitle && (
          <div className="">
            <h2 className="font-potta-one text-blue text-2xl  lg:text-3xl mb-6">
              {data?.title}
            </h2>
          </div>
        )}
        ...
        {data?.tcbRamen?.items &&
          data?.tcbRamen?.items.map((i, index) => (
            <div key={i._key} className="max-w-md pb-6">
              ...
              {i.block && (
                <div className="bg-red/20 rounded-lg p-3 font-jost mt-6 text-gray-800 w-fit">
